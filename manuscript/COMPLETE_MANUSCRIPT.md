...
> Manuscript v0.21 · generated 2026-08-18 19:00 UTC · private working-repository commit `e87f0d6836dd3f865282c7ffbfa330963010713d`
...
We tested the reach of the practice rather than assuming it. Every work citing the three primary hardware sources was retrieved through the OpenAlex citation graph on 18 August 2026 — 262 unique records, screened to 65 wireless-relevant works and then to **21 distinct flexible-metasurface or reconfigurable-surface system studies**, of which **14 were read in full and 7 are behind publisher paywalls with no open deposit located**. Every count below is therefore a **lower bound**, and §8.5's closing paragraphs state precisely what that bound does and does not permit. Method, seeds, queries and the per-work screening table are in Section 2.3 and the supporting documentation.
...
**The observation window is 14 of 21 studies, and the finding is stated inside it.** Among the fourteen candidate system studies whose full texts were read, five instances of scope change occur, in four papers. Seven further studies could not be read — publisher paywalls, no open deposit — and an abstract-level scan establishes nothing here, because propagation occurs only in body text: scanning all 65 stage-one abstracts for timing terminology returns no hits at all. **An instance among the seven unread studies cannot be excluded, and neither can an author-disjoint one.** Every count in this section is a lower bound on a partially observed set, and no proportion, rate or prevalence is estimated from it.

One of the seven illustrates why the bound is stated in terms of full-text access rather than of authorship. A study of flexible reconfigurable surfaces for covert communication [33] is disjoint from the FIM system-paper lineage, and on authorship alone would be a candidate author-disjoint case in either direction. Its published-record metadata, however, shows that it shares an author with the flexible microwave metasurface [6] — one of the three primary hardware sources — so it is not author-disjoint in the sense used here. That is all its metadata can establish. Whether it attaches a timing value to the hardware it cites is unknown to us, because we could not read it, and we make no claim either way.
