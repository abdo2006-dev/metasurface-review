...
### U13 — Two source-record entries corrected by this audit
...
Both corrections are recorded in `CHANGELOG.md`.

### U14 — Two files in the corpus were mislabelled or unlabelled
The array-level Harz paper of 2022 is a previously unrecorded second work by the same group and is not the same paper as the 2020 element design. Both are now listed separately in `source_inventory.md` and carry distinct bibliography entries.

### U15 — All previously missing sources are now resolved
The Harz element paper, the cylindrical conformal RIS paper, both conformal metasurface papers by the Nanjing group, and the wide-angle conformal active metasurface paper were all already in the corpus (M01–M05). **M06, the Fan Li supplement, was retrieved on 17 August 2026 and is now archived.** The missing-source list is therefore closed in full.
