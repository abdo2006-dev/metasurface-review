...
- **‼ Timing evidence — RE-VERIFIED AGAINST THE PDF 22 August 2026, and the previous entry was wrong.** It recorded the sub-0.1 ms figure as **per-element**. It is not. Full-text search of the 10-page article returns **"0.1 ms" exactly once**, in the abstract (p. 1): *"The proposed RIS enables real-time reconfiguration, with each element controlled through an FPGA achieving an update time below 0.1 ms."* The only body-text quantities that can support it are on **p. 6**: *"For the 16 × 16 RIS tile, once a configuration is stored in the FPGA, the internal update is completed in approximately 40 ns. Including the external control link, the end-to-end reconfiguration time is about 40 μs when using the LAN or Wi-Fi (Ethernet-based) interfaces, and up to 100 μs when using the USB/UART serial interface."* **100 μs = 0.1 ms.** The abstract figure is therefore the **end-to-end reconfiguration time of one 16 × 16 tile over the USB/UART serial interface**, not a per-element measurement. The strings "per-element" and "per element" **do not occur in the paper**.
  - **Start event:** a configuration command issued by the host over the external control link. **End event:** end-to-end reconfiguration complete — the FPGA → latch → diode-bias write path. **No electromagnetic end criterion is stated.**
  - **The PIN-diode electromagnetic state transition is not included and is not measured anywhere in the paper.** Full-text search returns **no** occurrence of "switching time", "rise time" or "settl-" of any form. PIN diodes are described qualitatively as providing "fast electronic tuning capabilities" (p. 2), with no number.
  - **Second bound:** *"The tile ON-OFF configuration can be updated in less than 10 ms"* (**p. 8**), repeated in the conclusion as *"update times below 10 ms via FPGA"* (**p. 9**). Scope is not restated there; the object in context is the same 16 × 16 tile.
  - **Consequence: both reported bounds concern the same object — one 16 × 16 tile — and are consistent as bounds** (100 μs < 10 ms). They are not two scopes of one quantity, and the corpus must not present them as an element-versus-tile contrast. That reading was introduced by this project, not by the source, and is withdrawn (`CHANGELOG.md` CH-151).
  - Control architecture note (p. 6): time-multiplexing "reduces the effective update rate to one quarter of the raw parallel speed."
...
|---|---|---|---|---|---|---|
| **MA-26** | A Survey on Reconfigurable and Movable Antennas for Wireless Communications and Sensing | copy held is arXiv:2602.17977 (39 pp.). ‼ **Version of record confirmed 3 Sep 2026, correcting the 17 Aug entry that reported none:** *IEEE Commun. Surveys Tuts.* **28**:4842–4882, 2026, DOI 10.1109/COMST.2026.3663021. **Cite the version of record, with section locators** — the page range is known and recorded, but the version-of-record full text was not obtained, so passage-specific page locators into it could not be checked and preprint page numbers must not be carried into it; U11's caveat on the zero-occurrence "FIM" search stands, since it is a property of the copy held | A6 review | review synthesis | closest conceptual review; source of the instantaneous-vs-statistical-CSI and hierarchical-control positions | IG-11 |
...
| **YOO-21** | Design of Conformal Array of Rectangular Waveguide-fed Metasurfaces | copy held is arXiv:2109.09450 (6 pp.). ✅ **Version of record confirmed 3 Sep 2026:** *IEEE Trans. Antennas Propag.* **70**(7):6060–6065, Jul. 2022, DOI 10.1109/TAP.2022.3140822. **The bibliography must give 2022**; the internal key's "21" is the preprint year and is never a citation. Cited once, without a locator, so no locator migration is required | A-conformal fed antenna | reduced-order + CST | conformal reduced-order modelling precedent | IG-19 |
...
**Corpus totals (revised 3 September 2026):** **34 PDF files** → **26 unique documents** → **24 distinct research contributions** (distinct, not independent: several share authors — see §4 and `audits/independence_audit.md`).

The file count rose by four, and then by three more when the published full texts of `01_Ranasinghe`, `04_Yang` and `07_Xu` were obtained on 17–18 August 2026 and archived alongside the preprints as `01V`, `04V` and `07V`. **The document and contribution counts did not change** in either step. A version of record and its preprint are two versions of one document, so the unique-document total stays at 26 and the contribution total at 24; only the file count moves. Supplementary information and peer-review files belong to their parent papers and are not independent studies; this is the project brief's independence rule and it applies to material this review retrieved just as it applies to material it found. The four new files are evidence *about* LI-25 and BAI-22, not evidence alongside them, and they are assigned to independence groups IG-5 and IG-8 accordingly.
...
| **MA-26** | ‼ **CORRECTED 3 Sep 2026 — a version of record does exist and the 17 Aug check missed it.** *IEEE Commun. Surveys Tuts.* **28**:4842–4882, 2026, DOI 10.1109/COMST.2026.3663021; the Crossref record was created 9 Feb 2026 and deposited 23 Feb 2026, six months before the check | ✗ **metadata only** — the publisher's full text is paywalled and no open deposit of the published version was located | ✅ held preprint read in full | ✅ E-40…E-43 verified in the held copy | **the version of record**, with locators given as **section numbers** because the version-of-record full text was not available to us and passage-specific page locators into it could not be checked | ⚠ the preprint page locators previously used (pp. 3, 20, 24–25, 26–27) were **withdrawn** and replaced by §I-C, §III-B4, §IV-B, §V-A and §V-C; the zero-occurrence "FIM" search remains a property of **the copy held**, and U11's caveat stands |
...
|---|---|---|---|---|---|
| [11] | MA-26 | preprint held and **read in full**; a version of record exists (confirmed 3 Sep 2026) but its full text is paywalled | the instantaneous-vs-statistical-CSI position, hierarchical control, the "mechanical is slower" dichotomy, the movement-time modelling recommendation | **A** | **Action taken 3 Sep 2026.** Cited at the version of record. The published record and page range (28:4842–4882) were verified; the version-of-record full text was not obtained, so passage-specific page locators into it could not be checked and every locator was re-expressed as a section number and each passage re-verified in the held preprint: §I-C (mechanical movement typically slower), §III-B4 (instantaneous vs statistical CSI, quasi-static vs fast fading), §IV-B (conflicting sensing and communication requirements), §V-A (hierarchical and cross-layer control), §V-C (inertia, settling time, friction, gear backlash). **Preprint page numbers were not carried across.** |
| [13] | LFIM-26 | **identified bibliographically only**; ✅ version of record confirmed 3 Sep 2026 — *Proc. IEEE ICC*, May 2026, pp. 1–6, DOI 10.1109/ICC59461.2026.11588269, cited at it | cited twice, jointly with [12], solely for the *existence* of published statistical-CSI FIM optimisation | **B** | **Retained, with the evidence level made explicit.** [12] was read in full and independently supports the same existence claim, so no claim rests on [13] alone. Note that [12] and [13] share three authors and are one line of work — recorded, and never presented as two confirmations |
...
| [29] | ANJ-MIMO | author version **read in full** | Premise 2 (the coherence-block assumption), Table 5 row 35, Table 8 row 2, counterexample N2 | **A** | **No action needed** — the author version was read in full, and the version of record is confirmed as *IEEE Trans. Commun.* **73**(9):7349–7365, Sep. 2025 (Crossref, 19 Aug 2026). The bibliography cites the published record |
...
‼ **arXiv:2606.06845 (Hu et al., multicell FIM — source of the Table I feasibility claim audited in §5.2) is no longer in this category.** The published version of record — *IEEE Trans. Wireless Commun.*, vol. 25, pp. 18579–18595, 2026, doi:10.1109/TWC.2026.3701359 — was obtained on 3 September 2026 and archived as `external_sources/HU-26_Weighted_Sum_Rate_FIM_Multicell_IEEE_TWC_2026_VERSION_OF_RECORD.pdf`. The §5.2 tracing was re-run against it and every quoted value survives at the published locators; one **new** finding emerged, a bibliographic mapping discrepancy inside Table I itself (`evidence_matrix.md` E-FP-02b, Supplementary Table S8 row P0). **It is an external source, not part of the reviewed set**: the extraction protocol of §2 was never applied to it, it is not coded on the adaptation-chain matrix, and it enters no corpus total — see `external_sources/README.md`. Still in this category: arXiv:2512.23045 (Kumar et al., statistical-CSI FIM), arXiv:2502.16478 (An et al., FIM MIMO, IEEE TCOM), arXiv:2510.07466, arXiv:2601.15471, arXiv:2508.00268, arXiv:2605.29227, arXiv:2506.23052, arXiv:2511.00878, arXiv:2510.24190. **These were inspected online only.** Any claim resting on them is marked `[EXTERNAL — ABSTRACT/HTML INSPECTED, PDF NOT ARCHIVED]`.
