...
Ten stages (S1–S10 in `adaptation_chain_matrix.md`), each classified per source as demonstrated / measured / simulated / assumed / absent. The result — that **no source measures more than five of the ten stages, that none measures channel estimation (S3) together with a commanded shape change (S7), and that the measured stages are distributed across four physically non-equivalent architectures whose timing quantities are not directly interchangeable** — is not stated anywhere located in this audit. MA-26 recommends that such quantities be modelled; it does not map which are measured and on what.

⚠ **Wording constraint (17 August 2026).** This contribution was previously stated as "no source records more than five **contiguous** stages". **That word was mathematically false and must not return.** The largest linked set, LI-25's, covers S1–S2 and S4–S6 — it skips S3, so it is not contiguous under our own taxonomy. The contribution does not depend on contiguity and is not weakened by dropping it: what makes the decomposition informative is *which* stages are missing and that the measured ones sit on incompatible platforms, not how many sit in an unbroken row. Acceptable forms: "the largest linked set of stages measured on one platform"; "five of the ten stages"; "no source measures S3 together with S7". Forbidden: "five contiguous stages", "longest contiguous chain", "contiguous run".
