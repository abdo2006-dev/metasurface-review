...
**Bearing on the manuscript's claims.** G1 is **narrowed** and stated for the commanded case specifically (§5.3). No numerical value in Table 2 or Supplementary Table S1 is contradicted. The 240 s figure is **not** ranked against the register: it is a different timed object, a different actuation mechanism and a different interval, and the comparability rules of this review forbid the comparison. It is also not far above the register's own slowest mechanical value — Bai's closed-loop convergence is approximately 2.5 minutes — so any "orders of magnitude" framing would be wrong on the arithmetic as well as on the class. What the figure establishes is that a geometric change has been timed on a radiating radio-frequency stack. C1–C4, G2, G3, G4, the A1–A7 taxonomy, the S1–S10 decomposition and every count are unaffected.
